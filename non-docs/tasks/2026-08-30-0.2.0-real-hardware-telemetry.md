...
- [x] **Step 1.1 Contracts (`src/telemetry/types.ts`)**:
  - ประกาศ `TelemetryMode = 'LIVE' | 'MANUAL'`
  - กำหนด Interface `HardwareTelemetryProvider` (methods: `start()`, `stop()`, `getLatestMetrics()`, `isSupported()`, `getStatus()`)
- [x] **Step 1.2 WebMock Provider (`src/telemetry/providers/WebMockTelemetryProvider.ts`)**:
  - พัฒนา Mock Provider สำหรับใช้บน Web Browser และ Vitest
- [x] **Step 1.3 Native Provider (`src/telemetry/providers/NativeTelemetryProvider.ts`)**:
  - พัฒนา Native OS Provider พร้อม Fallback ปลอดภัยหากเรียกใช้นอก Desktop Environment
- [x] **Step 1.4 Telemetry Manager (`src/telemetry/telemetryManager.ts`)**:
  - Singleton จัดการ Lifecycle การเริ่ม/หยุด Polling และสลับ Mode LIVE/MANUAL

---

### 🔹 Phase 2: Store Integration & React Hook
- [x] **Step 2.1 Store Telemetry Extension (`src/stores/companionStore.ts`)**:
  - เพิ่ม `telemetryMode: 'LIVE' | 'MANUAL'` ใน State
  - Action `setTelemetryMode(mode)` เพื่อสลับการรับสัญญาณ
- [x] **Step 2.2 Telemetry Hook (`src/telemetry/useTelemetry.ts`)**:
  - Custom React Hook จัดการ Auto-start/cleanup เมื่อ Component Mount/Unmount

---

### 🔹 Phase 3: UI Controls & Showcase Update
- [x] **Step 3.1 Mode Switcher in Controller (`MockMetricController.tsx`)**:
...
- [x] **Step 3.2 Live Telemetry Badges in HUD (`App.tsx`)**:
  - แสดง Telemetry Mode, Source Provider Badge (`🖥️ Windows Native` / `🌐 Web Simulation`), และ Status Indicator (`🟢 POLLING` / `⚪ IDLE`)

---

### 🔹 Phase 4: Automated Testing & 5-Pillar Verification
- [x] **Step 4.1 Unit Tests**:
  ...
  - `src/__tests__/useTelemetry.test.tsx`
  - `src/__tests__/MockMetricController.test.tsx`
  - `src/__tests__/App.test.tsx`
- [x] **Step 4.2 5-Pillar Automated Gate**:
  - `npm run verify` (Lint, Format, Typecheck, Test, Build) ผ่านครบ 100% (12 test files / 84 tests)

---

## 5. ✅ Acceptance Criteria
- [x] สลับระหว่าง Live Mode และ Manual Simulation Mode ได้อย่างไร้รอยต่อ
- [x] ขณะอยู่ใน Live Mode ค่า CPU, RAM, Disk จะอัปเดตทุก 1.5–2.0s และขับเคลื่อนท่าทางของ Lo-fi Bun ได้อย่างถูกต้อง
- [x] ระบบ Hysteresis $\pm 3\%$ ทำงานราบรื่น ไม่เกิดอาการภาพกระตุกสลับไปมาเมื่อค่าแกว่ง
- [x] รันบน Web Browser ปกติได้โดยไม่เกิด Runtime Error (Graceful Fallback)
- [x] ทุก Test Suites ใน `npm test` ผ่าน 100% และ `npm run verify` สำเร็จสมบูรณ์
