...
- `lofi-bun-companion/package.json` — Vite + React 18 + TypeScript + Tailwind CSS + Vitest + ESLint + Prettier
...
- `lofi-bun-companion/src/types/companion.ts` — 6 Animals Data Registry & State types
- `lofi-bun-companion/src/stores/companionStore.ts` — State Store & Priority Evaluator
- `lofi-bun-companion/src/components/PetRenderer/PetSprite.tsx` — GPU CSS Step Animator
...
- `lofi-bun-companion/src/__tests__/companionStore.test.ts` — Vitest unit tests verifying state transitions

---

## 4. 🛠️ Implementation Steps (ขั้นตอนการทำ)
- [ ] **Step 1**: Initialize Vite React TypeScript project in `lofi-bun-companion/` with Tailwind CSS, Vitest, ESLint, and Prettier (configure `npm run verify`).
- [ ] **Step 2**: Define TypeScript types & Pluggable Registry schema (`types/companion.ts`) starting with **Lo-fi Bun**.
- [ ] **Step 3**: Implement Zustand companion store (`stores/companionStore.ts`) with state priority logic.
- [ ] **Step 4**: Build `PetSprite.tsx` with clean CSS `steps()` and pixelated rendering for Lo-fi Bun (Zero CPU idle overhead).
...
- [ ] แอนิเมชันของ **Lo-fi Bun** ขยับนุ่มนวลแบบ Pixel Art 100% GPU
