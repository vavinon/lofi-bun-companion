...
- [ ] **Step 1.1 Scaffolding**: สร้างโครงสร้างโปรเจกต์ React 18 + TypeScript ภายใน `lofi-bun-companion/`
- [ ] **Step 1.2 Dependencies & Scripts**: กำหนด `package.json` พร้อม 5 Quality Scripts:
  ```json
  "scripts": {
    "dev": "vite",
  ...
  ```
- [ ] **Step 1.3 Quality Configurations**:
  - [ ] **ESLint v9 Flat Config (`eslint.config.js`)**: กำหนดค่า `@eslint/js`, `typescript-eslint`, และ `eslint-plugin-react-hooks`
  - [ ] **Prettier (`.prettierrc`)**: `{"semi": true, "singleQuote": true, "tabWidth": 2, "trailingComma": "es5"}`
  - [ ] **Strict TypeScript (`tsconfig.json`)**: `{"compilerOptions": {"strict": true, "noUnusedLocals": true, "noUnusedParameters": true, "target": "ES2022", "moduleResolution": "bundler"}}`
- [ ] **Step 1.4 Runtime & Test Setup**:
  - [ ] **Vite & Vitest (`vite.config.ts`)**: `plugins: [react()]`, `test: { globals: true, environment: 'jsdom' }`
  - [ ] **HTML Entry (`index.html`)**: โหลด Google Fonts (`Outfit` / `Inter`) และเพิ่ม CSS token `image-rendering: pixelated;`
- [ ] **Step 1.5 Baseline Verification**: ทดสอบรัน `npm run verify` รอบแรกเพื่อให้มั่นใจว่า Pipeline เครื่องมือพร้อม 100%

---

### 🔹 Phase 2: Domain Types & Decoupled State Machine (TDD Approach)
- [ ] **Step 2.1 Domain Types (`src/types/companion.ts`)**:
  ```ts
  ...
  ```
- [ ] **Step 2.2 Hysteresis Smoothing Function (`src/utils/hysteresis.ts`)**:
  * **CPU Thresholds**:
  ...
  * **Disk Threshold**:
  ...
  ```
- [ ] **Step 2.3 State Priority Evaluator (`src/utils/stateResolver.ts`)**:
...
- [ ] **Step 2.4 Automated Unit Tests (Vitest)**:
  - [ ] `src/__tests__/hysteresis.test.ts` (Boundary Cases: 16.9%, 17%, 20%, 56.9%, 57%, 60%, 71.9%, 72%, 75%)
  - [ ] `src/__tests__/stateResolver.test.ts` (Priority Combinations: Rest Override, Disk Priority, Heavy RAM Flag)
- [ ] **Step 2.5 Unit Test Verification**: รัน `npm test` เพื่อตรวจสอบให้ผ่าน 100%

---

### 🔹 Phase 3: Zustand State Store
- [ ] **Step 3.1 Reactive Store Implementation (`src/stores/companionStore.ts`)**:
  ```ts
...
- [ ] **Step 3.2 Store Integration Tests (`src/__tests__/companionStore.test.ts`)**:
  - [ ] เขียนเทสต์ทดสอบการอัปเดตค่าและการแจ้งเตือน Subscribers
- [ ] **Step 3.3 Test Suite Verification**: รัน `npm test` ตรวจสอบ Store ผ่าน 100%

---

### 🔹 Phase 4: Vector Pixel-Art & GPU CSS Step Animator
- [ ] **Step 4.1 Vector Spritesheet (`src/assets/sprites/bun-sprites.svg`)**:
...
- [ ] **Step 4.2 GPU CSS Step Animator (`PetSprite.tsx` & `PetSprite.module.css`)**:
  ```css
  .spriteCanvas {
    width: 64px;
    height: 64px;
  ...
  ```
- [ ] **Step 4.3 Head Prop Layer (`PropLayer.tsx`)**:
  ```css
  .propCarrot {
    position: absolute;
    top: -8px;
...
- [ ] **Step 5.1 Glassmorphism Controller (`MockMetricController.tsx` & `.module.css`)**:
  * Slider ควบคุม CPU Usage (0–100%, Accent: `#FF9A3C`)
  * Slider ควบคุม RAM Usage (0–100%, Accent: `#A3B899`)
  * Slider ควบคุม Disk Usage (0–100%, Accent: `#8EA8C3`)
  * Toggle สวิตช์ Rest Mode (Force Rest, Accent: `#DDD5E9`)
  * Glassmorphism Tokens: `background: rgba(46, 34, 31, 0.65)`, `backdrop-filter: blur(12px)`, `border: 1px solid rgba(255, 248, 240, 0.12)`
- [ ] **Step 5.2 Showcase Container (`App.tsx` & `App.module.css`)**:
  * จัดเลย์เอาต์แผ่นรองโต๊ะ Lo-fi Desk Mat (`#1C1614` Dark Espresso)
  * ป้ายสถานะเรียลไทม์ (Active State, RAM Prop, FPS)
- [ ] **Step 5.3 Main Entry Point (`main.tsx`)**: ผูก React DOM Tree เข้ากับ Root Container

---

### 🔹 Phase 6: Full 5-Pillar Verification & Final Delivery
- [ ] **Step 6.1 5-Pillar Automated Gate**:
  - [ ] `npm run lint` (ESLint 0 errors, 0 warnings)
  - [ ] `npm run format:check` (Prettier 100% compliant)
  - [ ] `npm run typecheck` (`tsc --noEmit` 0 errors)
  - [ ] `npm test` (Unit & Integration Tests ผ่านครบ 100%)
  - [ ] `npm run build` (Vite Build Bundle สำเร็จสมบูรณ์)
- [ ] **Step 6.2 Combined Gate**: รัน `npm run verify` ผ่านแบบ One-Shot 100%
- [ ] **Step 6.3 Devlog Entry**: บันทึกรายงานใน `non-docs/devlogs/2026-08/2026-08-30.md`
...
- [ ] **Hysteresis Smoothing**: เลื่อน Slider ใกล้จุดตัด 20%, 60% และ 75% แล้วแอนิเมชันไม่กระตุกสลับไปมา
