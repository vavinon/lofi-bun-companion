...
- [x] **[P2-7]** สร้าง `src/assets/sprites/cockatiel-sprites.svg` (256x320 px)
...
- [x] **[P2-8]** สร้าง `src/assets/sprites/prop-vinyl.svg` (28x20 px)
  - กองแผ่นเสียงไวนิล 4 แผ่นซ้อนกัน มีฉลากส้ม-เหลือง-ทีลเรโทร และประกายโน้ตดนตรี Groove Overlay

- [x] **[P2-9]** สร้าง `src/assets/sprites/dolphin-sprites.svg` (256x320 px)
...
- [x] **[P2-10]** สร้าง `src/assets/sprites/prop-coral.svg` (28x20 px)
  - กิ่งปะการังสีชมพูประดับด้วยไข่มุกเรืองแสงและเปลือกหอยมุกหลากสี สำหรับสวมบนหัว Wave Dolphin เมื่อ RAM > 80%

#### Acceptance Criteria (Phase 2)

- [x] SVG Spritesheet มี `viewBox="0 0 256 320"` และ Prop มี `viewBox="0 0 28 20"` ถูกต้องครบถ้วนทั้ง 6 ตัวละคร
- [x] Frame Grid: แต่ละ State Row ครอบคลุม 4 Frames เรียงแนวนอน (256px รวม, 64x64 px ต่อเฟรม)
- [x] `image-rendering: pixelated` และ `shape-rendering="crispEdges"` คมชัด ไร้เบลอเมื่อ Scale ขึ้น 2x–4x
- [x] Prop Overlay (28x20 px) วางซ้อนบนหัวตัวละครเมื่อ RAM > 80% แล้ว ไม่บดบัง Face Expression ของ State หลัก
- [x] ทุก SVG Asset ผ่าน XML Well-Formedness Gate และ Integrity Test (`svgAssets.test.ts`) 100%
...
- [x] **[P3-1]** อัปเดต `src/components/Pet/PetSprite.tsx` — Dynamic Sprite Loader
...
- [x] **[P3-2]** อัปเดต CSS Module (`PetSprite.module.css`) สำหรับ Dynamic Background
...
- [x] **[P3-3]** ตรวจสอบว่า Prop Overlay สลับ `propUrl` ตามตัวละครที่เลือกอย่างถูกต้อง
  - แสดง Prop เมื่อ `resolvedState.isHeavyRam === true` เท่านั้น
  - ซ่อน Prop เมื่อ RAM กลับมาปกติ (<80%)

- [x] **[P3-4]** (Optional) เพิ่ม Smooth Companion Transition
...
- [x] **[P4-1]** อัปเดต `src/components/Desktop/PetContextMenu.tsx` — Switch Companion Submenu
...
- [x] **[P4-2]** อัปเดต Full Dashboard (`src/App.tsx`) — Character Selector Ribbon
...
- [x] **[P4-3]** อัปเดต `src/components/Desktop/CompactMascotView.tsx` — Active Companion Pill
...
- [x] Context Menu Submenu แสดง 6 ตัวละครถูกต้อง พร้อม Active Indicator บนตัวที่เลือก
- [x] กด Companion ใน Context Menu → ตัวละครบน `CompactMascotView` สลับทันที (<100ms latency)
- [x] Full Dashboard Character Ribbon แสดง Card ทั้ง 6 ตัว Active State ไฮไลต์ถูกต้อง
- [x] `CompactMascotView` แสดงชื่อตัวละคร Active อย่างถูกต้อง
- [x] UI ไม่มี Layout Shift หรือ Flicker เมื่อสลับตัวละคร
...
- [x] **[P5-1]** สร้าง `src/__tests__/companionRegistry.test.ts` — Registry Integrity Tests
...
- [x] **[P5-2]** อัปเดต `src/__tests__/companionStore.test.ts` — Companion Switcher Tests
...
- [x] **[P5-3]** อัปเดต `src/__tests__/PetSprite.test.tsx` — Dynamic Renderer Tests
...
- [x] **[P5-4]** อัปเดต `src/__tests__/PetContextMenu.test.tsx` — Switcher UI Tests
...
- [x] **[P5-5]** รัน 5-Pillar Quality Gate ให้ผ่าน 100%
  - [x] `npm run lint` — 0 errors, 0 warnings
  - [x] `npm run format:check` — Prettier 100% matched
  - [x] `npm run typecheck` — `tsc --noEmit` 0 errors
  - [x] `npm test` — Vitest All suites pass, 0 failures (Test count >= 130+)
  - [x] `npm run build` — Vite Production Bundle built successfully
...
- [x] สลับตัวละครทั้ง 6 ตัวได้อย่างลื่นไหลจากทั้ง Context Menu และ Dashboard Ribbon
- [x] ทุกตัวละครเล่นแอนิเมชัน 5 ท่า (`IDLE`, `FOCUS`, `FRENZY`, `DISK`, `REST`) ถูกต้อง 100%
- [x] Prop Overlay (`HEAVY_RAM`) แสดงสิ่งของเฉพาะตัวของแต่ละ Companion อย่างถูกต้อง
- [x] Companion Name Pill ใน `CompactMascotView` อัปเดตสอดคล้องกับตัวละคร Active

### Performance Requirements

- [x] CPU 0.0% บน Idle mode — Pure CSS `steps()` animation ไม่มี JavaScript interval ขับเคลื่อน
- [x] RAM < 35MB รวม Asset SVG ทั้ง 12 ไฟล์แล้ว
- [x] Companion Switch Latency < 100ms (UI Response ไม่รู้สึก Lag)

### Quality Requirements

- [x] `npm run verify` ผ่าน 100% ทุก 5 Gate ด้วย 0 errors
- [x] Automated Test Count >= 130 tests (เพิ่มจาก 111 เดิม)
- [x] ไม่มี TypeScript `any` ที่ไม่จำเป็น ใน Registry หรือ Renderer ใหม่
- [x] ไม่มี `console.error` หรือ React Warning ใน Browser Console

### Documentation Requirements

- [x] อัปเดต `CHANGELOG.md` บันทึก `[1.1.0]` release ตาม Keep a Changelog 1.1.0 format
- [x] อัปเดต Character spec files สถานะ `🟡 Draft` -> `🟢 Specification Ready` หลัง Sprite สร้างเสร็จ
- [x] บันทึก Devlog session ใน `non-docs/devlogs/2026-08/2026-08-30.md`
